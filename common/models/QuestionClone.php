    ...
    public function saveQuestionClone($data)
    {
        ...
        foreach ($data as $elements) {
            foreach ($elements['question'] as $question) {
                if ($question) {
                    $dataInsert[] = [$question['q_content'], $question['q_type'], $elements['ut_id'],$question['q_id']];
                    $count++;
                }
            }
        }
        $db->createCommand()->batchInsert(self::tableName(), ['qc_content', 'qc_type', 'ut_id','q_id'], $dataInsert)->execute();
